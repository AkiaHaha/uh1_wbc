...
file_paths = ["xyz_wOpt_20kg.txt", "xyz_nOpt_20kg.txt"]  # Replace with the paths to your two files
data_list = []
...
plt.title("Err XYZ Comparison (Payload=20kg)", pad=0)  # Move the title down a little
plt.xlabel("Time (s)")
plt.ylabel("Err (m)")

# Move legend below the title and set it to one row
plt.legend(bbox_to_anchor=(0.5, 0.96), loc='upper center', fontsize=12, ncol=3, handlelength=2.5)
...
plt.savefig("exp4-xyz-compare-20kg.png", dpi=1000)
plt.show()
