...
file_paths = ["xyz_wOpt_5kg.txt", "xyz_nOpt_5kg.txt"]  # Replace with the paths to your two files
data_list = []
...
time_interp = np.linspace(min(time_data1.min(), time_data2.min()), max(time_data1.max(), time_data2.max()), 500)
...
plt.title("Err XYZ Comparison (Payload=5kg)", pad=40)  # Move the title down a little
plt.xlabel("Time (s)")
plt.ylabel("Err (m)")

# Move legend below the title and set it to one row
plt.legend(bbox_to_anchor=(0.5, 1.1), loc='upper center', fontsize=12, ncol=3, handlelength=2.5)
...
plt.savefig("exp4-xyz-compare-5kg.png", dpi=1000)
plt.show()
