...
RobotDynamicsBiped::RobotDynamicsBiped() {
    ...
    pelvisJacoTc.J = MatrixNd :: Zero(NJF, NJG);///< JacobianTc of floating-base pelvis.
    pelvisJacoTc.JdotQdot = VectorNd :: Zero(NJF);

    torsoJacoTc.J = MatrixNd :: Zero(NJF, NJG);///< JacobianTc of torso.
    ...
    floatingPelvisJoint = Joint(SpatialVector(0.,0.,0., 1.,0.,0.),  //                ^ Z
                                ...
                                SpatialVector(0.,0.,0., 0.,0.,1.), //                |___Pelvis, Pelvis (1.1) 
    ...
    idTorso = model->AddBody(idPelvis, Xtrans(Vector3d(0, 0.0, 0.0)), joint_Rz, torsoLink, "torso");///< Pelvis related to world sys. by pelvis
    // idPelvis = model->AddBody(idPelvis, Xtrans(Vector3d(0, 0.0, 0.1)), fixedSoleJoint, torsoLink, "torso");
...
//  calcCentroidalDynamicsDescriptors();
    ...
    pelvisJacoTc.J = pelvisJacob;
    pelvisJacoTc.JdotQdot = pelvisJDotQDot;
...
VectorNd RobotDynamicsBiped::estBodyPosInWorldAkia(const VectorNd& jointPos, const VectorNd& jointVel, const unsigned int& bodyId) {//Daniel 5.27
    ...
    {
        case 0:
            bodyPos = CalcBodyToBaseCoordinates(*model, qTemp, idTorso, Math::Vector3d::Zero(), false);
            break;
        case 1:
...
VectorNd RobotDynamicsBiped::estPointTwistWorld(const VectorNd& jointPos, const VectorNd& jointVel, const int& footType) {
    VectorNd qTemp = VectorNd::Zero(NJG);
    VectorNd qDotTemp = VectorNd::Zero(NJG);
