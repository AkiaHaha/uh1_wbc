...
//============================================================//
    ...
    bool HqpWbc::wbcSolve(){
        ...
        for (int i = 0; i < nLevel; i++){
            ...
            if(i>0){optVarStar.at(i) = optVarStar.at(i-1) + nspMat.at(i-1) * optVarStar.at(i);}
        }
        return true;
    }
...
//============================================================//
    ...
    bool HqpWbc::hqpUpdateLeveln(const int & iLevel){
        ...
        hessianMat.at(iLevel) = taskMatAll.at(iLevel).transpose() * taskMatAll.at(iLevel);///< Hessian matrix, H = A^T * A;
        hessianMat.at(iLevel).diagonal().array() += 0.001;///< Matrix damping;

        gradientVec.at(iLevel) = - taskMatAll.at(iLevel).transpose() * taskVecAll.at(iLevel);///< Gradient vector; g = - A^T * b;
